...
// Ratio Counter Component for 24/7 format
const RatioCounter = ({ first, second, duration = 2 }: { first: number; second: number; duration?: number }) => {
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [heroRes, servicesRes, testimonialsRes, companiesRes] = await Promise.all([
          api.get('/hero/active'),
          api.get('/services/active'),
          api.get('/testimonials/active'),
        ...
        setTestimonials([
          {
            _id: '1',
            quote: 'Innovate Solutions transformed our workflow with intuitive software.',
            authorName: 'John Doe',
            authorTitle: 'CEO',
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            >
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold mb-4 sm:mb-6 leading-tight">
                {hero?.headline || 'Build Scalable Software Solutions That Drive Growth'}
              </h1>
              <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-blue-100 mb-6 sm:mb-8 leading-relaxed">
                {hero?.subheadline || 'We deliver enterprise-grade software solutions that help businesses innovate, scale, and succeed in the digital age.'}
              </p>
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6 sm:mb-8">
                ...
                >
                  ...
                  >
                    {hero?.ctaText || 'Start Your Project'}
                  </Link>
                </motion.div>
                <motion.div
                  whileHover={{ scale: 1.05 }}
